...
/*
 * This file contains settings that are shared across the entire project (including modules).
...
return [
    ...
    */
    ...
    /*
    │-----------------------------------------------------------------------------
    │ Logging level for framework system alerts
    │-----------------------------------------------------------------------------
    │
    │ For some data operations, the framework may send messages to the log
    │ if these external operations look suspicious.
    │ Contains #security_message tag for searching through the log list.
    │ It will also be used for all similar cases.
    │ This parameter contains the expected logging level for such messages.
    │ Default: 'warning'
    │
    │
    │-----------------------------------------------------------------------------
    │ Уровень логирования для системных оповещений фреймворка
    │-----------------------------------------------------------------------------
    │
    │ При некоторых операциях с данными фреймворк может отправлять сообщения в лог
    │ если эти внешние операции выглядят подозрительно.
    │ Содержит #security_message тег для поиска по списку логов.
    │ Также будет использовано для всех подобных случаев.
    │ Этот параметр содержит ожидаемый уровень логирования для таких сообщений.
    │ Изначально: 'warning'
    │
    */
    ...
    */
    ...
    │
    │-----------------------------------------------------------------------------
    │ Устанавливает часовой пояс для всех функций даты/времени в скрипте
    │-----------------------------------------------------------------------------
    │
    ...
    */
    ...
    /*
    │-----------------------------------------------------------------------------
    │ Registering functions in TWIG
    │-----------------------------------------------------------------------------
    │
    │ By default, TWIG doesn't detect functions that can be used in it,
    │ including global PHP functions with rare exceptions. Here, you can specify
    │ the name of a function to register in TWIG with the same name.
    │ This is an optional parameter, remove it if TWIG will not be used.
    │ Default: ['csrf_token', 'url']
    │
    │
    │-----------------------------------------------------------------------------
    │ Регистрация функций в TWIG
    │-----------------------------------------------------------------------------
    │
    │ По умолчанию TWIG не обнаруживает функции, которые в нём можно использовать,
    │ это касается и глобальных PHP функций с редким исключением. Здесь можно
    │ указать название функции для регистрации в TWIG с аналогичным названием.
    │ Это необязательный параметр, удалите его, если TWIG не будет использован.
    │ Изначально: ['csrf_token', 'url']
    │
    */
