...
final class MainTemplate
{
    ...
    public function __construct(string $path, array $template = [], bool $return = false ) {
        ...
        if ($GLOBALS['HLEB']['PROJECT_DEBUG_ON']) {
            ...
            Info::insert('Templates', (isset($GLOBALS['HLEB']['MODULE_NAME']) ? ' module `' . $GLOBALS['HLEB']['MODULE_NAME'] . '` ' : '') . $templateName . $backtrace . ' (includeTemplate) load: ' . (round($time, 4) * 1000) . ' ms');
        }
    }
    ...
    private function getTemplateDirectory($templateName) {
        if (!empty($GLOBALS['HLEB']['OPTIONAL_MODULE_SELECTION'])) {
            ...
            return HLEB_GLOBAL_DIRECTORY . '/modules/' . $GLOBALS['HLEB']['MODULE_NAME'] . "/" . $templateName;
        }
