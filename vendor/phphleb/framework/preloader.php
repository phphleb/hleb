...
if (!function_exists('hl_preliminary_exit')) {
     ...
     * @internal
     */
    function hl_preliminary_exit($text = ''): void
    {
        exit($text);
    }
}
...
$GLOBALS['HLEB']['PROJECT_DEBUG_ON'] = (bool)(HLEB_PROJECT_DEBUG && $_SERVER['REQUEST_METHOD'] === 'GET' && (empty($_GET['_debug']) || $_GET['_debug'] === 'on')) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH'] ?? '') !== 'xmlhttprequest';
...
defined('HLEB_PROJECT_STORAGE_DIR') or define('HLEB_PROJECT_STORAGE_DIR', (defined('HLEB_STORAGE_DIRECTORY') ? rtrim(HLEB_STORAGE_DIRECTORY, '\\/ ') : HLEB_GLOBAL_DIRECTORY . DIRECTORY_SEPARATOR . 'storage'));
...
if (!function_exists('hleb_system_log')) {
    ...
    function hleb_system_log($message, $level = 'error') {
        Log::getInstance()->log($level, $message);
    }
}
...
if (is_dir(HLEB_VENDOR_DIRECTORY . '/phphleb/radjax/')) {
    ...
    $GLOBALS['HLEB']['MAIN_DEBUG_RADJAX'] = [];
...
if (empty($radjaxIsActive)) {
    ...
    $GLOBALS['HLEB']['ENDING_URL_ON'] = in_array(strtolower($_SERVER['REQUEST_METHOD']), HLEB_ENDING_URL_INCLUDING_METHODS);
    $GLOBALS['HLEB']['SYSTEM_ENDING_URL'] = ($GLOBALS['HLEB']['ENDING_URL_ON'] ? HLEB_PROJECT_ENDING_URL : $address[strlen($address) - 1] === '/');
